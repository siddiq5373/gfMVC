...
<cffunction name="init" access="public">
	<cfargument name="urlPath" type="String" default="/assets/images/wordart/" hint="path with which stored images can be accessed from browser" />
	<cfargument name="artDir" type="String" default="#expandPath("/assets/images/wordart/")#" hint="absolute path of directory where generated images are to be stored" />
	<cfargument name="fontDir" type="String" default="#expandPath("/assets/files/fonts/")#" hint="absolute path of directory where you store fonts. this directory is checked for fonts before system's font directory is checked. when a font is missing from both the folders, 'serif' font is used." />
...
<cffunction name="getArtUrl" returntype="string" access="public">
	...
	<cfscript>
		...
		if (useCache eq true) {
			...
			//writeDump(local.imagePath);abort;
